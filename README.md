<div dir="rtl" style="line-height: 1.5;">
  
  <div align="center">
    <h1>🧠 Minecraft Bedrock AIChat</h1>
    فكرة هذا المشروع عبارة عن ربط بين ماين كرافت (Bedrock Edition) مع Gemini AI للتحدث مع اللاعبين، مع حل لمشكلة ظهور اللغة العربية في اللعبة.
    <div align="center"  >
      <img src="./chatExample.jpeg" alt="chatExample" width="720"/>
    </div>
  </div>

## 🌟 الميزات
* **ذكاء اصطناعي:** يستخدم نموذج `gemini-3-flash` (الأسرع والأحدث من Google).
* **دعم اللغة العربية:** يتضمن نظام معالجة مخصص [`arabicFix.js`](./arabicFix.js) يقوم بتشكيل وعكس النص العربي ليظهر بشكل صحيح في بيئة اللعبة، مع الحفاظ على الأرقام والكلمات الإنجليزية في أماكنها.
* **سجل محادثة:** يتذكر البوت سياق المحادثة كامل كأنه لاعب طبيعي.

## 🎮 توافق الإصدارات (Versions Support)
يعتمد هذا المشروع بشكل أساسي على مكتبة [**bedrock-protocol**](https://github.com/PrismarineJS/bedrock-protocol) لذلك، فإن دعم البوت للإصدارات الجديدة **مرتبط بتحديث المكتبة الأصلية**.

## 🛠️ المتطلبات
* [Node.js](https://nodejs.org/) (الإصدار 14 أو أحدث).
* سيرفر ماين كرافت Bedrock يعمل.

## 🚀 التثبيت والتشغيل
1. **تثبيت الحزم المطلوبة:** افتح التيرمينال في مجلد المشروع واكتب:
   ```bash
   npm install bedrock-protocol dotenv
   ```
2. **الإعداد:** افتح ملف `.env.example` وقم بتعديل البيانات التالية في الأعلى:
   
   ```env
   serverHost=your-server.net
   serverPort=19132
   ```
4. **تجهيز الذكاء الاصطناعي:** اذهب لموقع [Google AI Studio](https://aistudio.google.com/apikey) ثم أنشئ مفتاح API والصقه في ملف `.env.example`:
   
   ```env
   GEMINI_API_KEY=AIzaXXXXXXXXX
   ```
   ثم قم بتغيير اسم الملف إلى `.env`
6. **التشغيل:**
   ```bash
   node index.js
   ```

## 📂 هيكلية الملفات
* `index.js`: الكود الأساسي للاتصال بالسيرفر وإدارة المحادثات.
* `arabicFix.js`: مكتبة بسيطة لمعالجة النصوص (Shaping & Reordering) لتوافق ماين كرافت لأنها لا تدعم اللغة العربية بشكل افتراضي.
* `skinData.json`: ملف يحتوي على بيانات سكن (Skin) البوت.

## ❤️ شكر وتقدير (Credits)
هذا المشروع لم يكن ممكناً بدون الأدوات الرائعة التي يوفرها مجتمع المصادر المفتوحة. شكر خاص لـ:
* مكتبة  [PrismarineJS](https://github.com/PrismarineJS): لتطويرهم مكتبة [`bedrock-protocol`](https://github.com/PrismarineJS/bedrock-protocol) التي يعتمد عليها هذا المشروع بالكامل للاتصال بخوادم ماين كرافت.
</div>
